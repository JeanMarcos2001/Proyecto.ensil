-- ============================================================
-- MIGRACIÓN DE EMAIL DE LA TABLA CITAS A ALUMNOS Y APODERADOS
-- Proyecto: Dashboard-EnsilWeb
-- ============================================================

-- 1. Eliminar la columna email de la tabla citas (si existe)
ALTER TABLE public.citas DROP COLUMN IF EXISTS email;

-- 2. Agregar la columna email a la tabla alumnos (si no existe)
ALTER TABLE public.alumnos ADD COLUMN IF NOT EXISTS email TEXT;

-- 3. Agregar la columna email a la tabla apoderados (si no existe)
ALTER TABLE public.apoderados ADD COLUMN IF NOT EXISTS email TEXT;


-- 4. Actualizar función registrar_reserva
...
BEGIN
  ...
  -- Insertar el alumno. Si NO es dependiente (es independiente), guardar el email en alumnos.
  ...
  -- Insertar la cita registrando tipo_cita y creado_en (sin el campo email)
...
-- 5. Actualizar función registrar_familia
...
BEGIN
  ...
  LOOP
    ...
    -- Insertar cita para ese alumno (sin el campo email)
    INSERT INTO public.citas (id_alumno, id_filial, fecha_cita, hora_cita, estado, creado_en, tipo_cita)
